...
This chapter explains how to deal with different types of data within R, as well as how to clean, manipulate, and save data. When we are dealing with data we will be using the object of a data frame to save the data within the temporary memory of R itself (shown in the environment). For this chapter, the tidyverse contains the set of packages we will use. The installation and calling of this package is demonstrated in the chunk below:

```{r}
# install.packages("tidyverse)
library(tidyverse)
```
The output from calling the tidyverse shows all of the different packages it loads into R at once and their most recent version. The core packages comprise a powerful suite of tools for importing, wrangling, analyzing, and visualizing data. 

## Data Frames

Within R itself, a data frame can be created in two different ways: using the `tibble()` or `tribble()` functions. Both functions do exactly the same thing using different syntax. The `tibble()` function uses the format `row = c(), row2 = c()`. The `tribble()` function uses a vertical format, with `~` denoting the row names. An example of how to the `tibble()` function is given in the code chunk below:
...
It is important to make sure each column is the intended data type. Functions (especially mathematical ones) require often require a certain data type as an input.

### Date Time Objects

Include in this section more information on how to deal with time series data and parse time series character strings

There is a special data type for dates and times available through the `lubridate` package. This package is automatically included in the tidyverse. There are three different data types associated with dates/times: a date data type signifying the day, month, and year (`<date>`); a time data type signifying the time within a day (`<time>`); and a date time which includes both (`<dttm>`).  The code chunk below demonstrates how to use the `make_datetime()` function to create time objects. Using this data type ensures that graphs over time are produced properly using ggplot2. The beauty of the tidyverse is that all these packages build off each other. 

```{r}
make_datetime(year = 2020, month = 11, day = 25)
```

Often, one of the hardest parts of working with a new data set is correctly changing however they keep the date/time into a date time object. The lubridate package is able to create these objects with a variety of input formats, making this process much easier. The code chunk below demonstrates different methods of creating `<dttm>` objects. 

```{r}
ymd('2017-01-02') #year month day function creates a date object
mdy("01/02/2017", tz = "UTC") # month day year
# adding a timezone makes this a date time object instead
dmy_hms("2 Jan 2017 10:00:00 PM") #year month day _ hour minute second
# these functions can also read full text and abbreviations
```

One of the other really useful parts of lubridate date/time objects is the ability to refer to different components of a date time object. If you have a complete date time object, `year()` returns the year, `month()` returns the month, `mday()` returns the day of the month, `wday()` returns the day of the week, and the same for `hour()`, `minute()`, and `second()`.
...
For a more in-depth explanation of factors, refer to [this chapter](https://r4ds.had.co.nz/factors.html) of R for Data Science. [@r4ds]
...
If the excel file has multiple sheets, the `sheet =` parameter allows you to select the sheet by setting it equal to the name of the sheet. If the excel file has blank lines at the beginning of the file you can use the `skip = ` parameter to skip lines of the save before it starts to read it. This is often necessary to skip white space at the top and ensure the .csv file correctly reads into a data frame. 
...
What is clean data? For data to be clean there must be a clear pattern of organization to how the data is stored. This concept is also commonly referred to as tidy data. There are three general rules for making a data set tidy:

* Each variable must have its own column
* Each observation must have its own row
* Each value must have its own cell

Table 1, shown in the code chunk below, is an example of a tidy data set. Each observation (pertaining to a country and a year) has its own row. Additionally, each variable has its own column. This table is included with the tidyverse as an example and used in R for Data Science.[@r4ds] 
...
Often, data when you receive it will not be in this format. The first step after successfully importing data is to clean it. Luckily, the tidyverse offers functions that allow you to rearrange data frames into a format better suited for analysis and plotting. I will go through several examples of different common problems with data, and what functions are best to fix them.

### Pivoting Data

First, we will discuss the two functions used for pivoting data. The code chunk below shows a sample data frame that has multiple issues which are able to be corrected through pivoting. Instead of having a column for each variable, the variable "type" indicates the variable. Columns 3 and 4, which both contain doubles, pertain to the year of the value. These are examples of common problems with data organization. 

```{r}
sample_table
```

These two functions in dplyr are each solve one of these problems: 
...
For more information and examples about pivoting refer to the [Stanford Data Wrangling book](https://dcl-wrangle.stanford.edu/pivot-basic.html) and the R for Data Science Chapter on [tidy data](https://r4ds.had.co.nz/tidy-data.html). [@stanford] [@r4ds]
...
This first example in the code chunk below demonstrates how to make a new numeric variable using two existing numeric variables. This creates a new column that is the number of cases divided by the total population that year. The second new variable, `ln_pop`, demonstrates how to apply a function to a numeric variable to create a new variable. In this case the function is the natural log. 
...
This next examples demonstrates how to use the `case_when()` function. The `case_when()` function is very useful when creating a new categorical variable based on other already existing variables. This function does not have any explicit parameters. Instead, there can be any number of logical statements (e.g. cases / population < 0.0001), the `~`, and the corresponding value if the statement evaluates as true. The code chunk below demonstrates the correct syntax.
...
This creates a variable that indicates the level of cases per capita. This creates the variable in the character data type. This variable is perfect for using the factor data type, as it has inherent ordered levels. The code chunk below demonstrates how to turn this variable into a factor. When using mutate, if you set something equal to a variable name you already have, it will overwrite that variable. 
...
In some cases, a data frame will have more variables than you need for your analysis. The `select(data, cols)` function allows you to choose rows and put them in a new data frame. The data parameter is the data frame to select columns from, and the cols is the name of the columns to select. The code chunk below demonstrates how to use the select function using Table 1 as an example. This can be chained with other data wrangling functions using the piping operator discussed in the next section. This is very useful when cleaning data as it allows you to easily discard data you do not need. You should always keep an original copy of the data as you received it in the case that you need to go back, but you should save a modified data frame with only the variables you need for analysis.
...
It is very important to make sure the names of the columns comply with the conventions of coding and clearly indicate the variable. Most data sets have a code book that lists each variable and what it represents, but these names are often do not effectively identify the variable without having to refer to the code book. To rename the columns in a data frame use the `rename()` function available with the tidyverse. The code chunk below demonstrates this using the data frame longer table, saved after the first pivoting exercise above. 
...
It is also very useful when dealing with numeric variables as well. The code chunk below demonstrates simple and more complex numeric filters. The conditional statements can be combined using `&` (and) and `|` (or) operators. While this simple example restricts the data frame to only one row, this function is much more powerful in large data sets. 
...
The `filter()` function is wonderful for selecting subsets of data, but it does not allow you to reorder the rows according to a variable. For this we use the `arrange()` function. The basic syntax for arrange is shown in the code chunk below. The first example shows the default ascending order the `arrange()` function uses.  The second example shows how to get it to use descending order by wrapping the column in the `desc()` function. The third example shows what happens when multiple variables are included in the `arrange()` function. The function arranges according to the first variable and then without violating the arrangement of the first variable arranges according to the second, and so on. The final two examples show how arrange deals with characters. The most important difference between `filter()` and `arrange()` is that `filter()` loses observations when `arrange()` keeps all observations but just reorders them. 
...
The summary function allows the creation of summary statistics of the data. These tables are vitally important in analysis and can be crucial elements of analysis that back up claims in written submitted work. The function can be called either through `summarize()` or `summarise()`. Both do the exact same thing. The code chunk below shows an example of the summary function. 
...
The summary function is also useful for determining the number of unique values in a variable. This can come in handy when visualizing data. The code chunk below demonstrates this feature using the `unique()` function. 

```{r}
table1 |> summarize(unique(country))
```

### Grouping Data

When dealing with data that has categorical variables, it makes sense to analyze the data in context of each group. This can be done through the `group_by()` function. `group_by()` on its own does not have any affect. It must be combined with another function. Usually it is best used in on a categorical variable. The code chunk below demonstrates how it can be applied to countries in table1. When used in conjunction, it groups all observations with the same country and gives stats for that instead of the data in totality as shown above. 
...
Often, for analysis it will make sense to combine data from different sources to allow for a greater scope to analysis. This process is called merging data. To merge a data set with another one they must be relational. This is done through matching keys: variables that uniquely identify observations. There are two types of keys:  
...
Keys are not mutually exclusive; a variable can be a primary and foreign key. If a data frame lacks a primary key, you may need to make one. This can be done using `mutate()` and `row_number()`: `mutate(data, unique_key = row_number())`. This makes the row number into a variable uniquely identifying each observation. This is called a surrogate key.
...
There are two types of joins for mutating joins:
...
* outer join
  + keeps all observations that appear in one or both of the data
  + `left_join(x, y, by = "key")` keeps all of the observations in the original table
  + `right_join(x, y, by = "key")` keeps all of the observations in the new table
  + `full_join(x, y, by = "key")` keeps all observation in both tables

Below, I use four fictional tables to demonstrate the mutating join functions discussed above.
...
The first thing you may notice is that in `original` and `new`, the column that contains the key is not the same. To join these you can either first rename the key columns so they have the same name or use the syntax of the first example. This is demonstrated in the code chunk below.
...
If the `by = ` parameter is not specified, `by = NULL`. This means that the key will be all variables that the two data frames share in common. This is called a natural join. Most of the time it is good practice to specify the key or keys you want to use 
...
Sometimes, one variable will not be able to uniquely identify observations as discussed previously. The `multiple` data frame and the code chunk below demonstrate the syntax of using multiple variables to identify observations.  
...
There are a variety of objects you may want to save in your code for later reference, either in a written work or another document. Graphs saved to images is other file format that is the most useful. That will be covered in the next chapter on data visualization. For other types of objects that you may want to save, look on the internet for the best file type and function to use to save them. Always check that the saved file is as you intended it to be and that it can be read into the format you desire. 
...
Tidycensus is an R package that allows users to easily access census data and estimates. A complete explanation census data and how to use tidycensus with R is given [here](https://walker-data.com/census-r/index.html) in Kyle Walker's *Analyzing US Census Data: Methods, Maps, and Models in R*.[@census] Walker provides a comprehensive and updated review of how to use Census data with R. This is an example of an API (Application Programming Interface). APIs are amazing tools for getting access to underlying data on the internet from a wide variety of sources. For more information on APIs refer [this chapter](https://dcl-wrangle.stanford.edu/api-basics.html) from Stanford's *Data Wrangling*.[@stanford] 

```{r}
library(tidycensus)
```

Tidycensus is an example of a R package that is a wrapper for an API, making the process of obtaining data easier. The census has a variety of APIs available, but the ones most applicable to your work are:
...
* Decennial Census
  ...
  + Most accurate population data, but only available every 10 year
...
* the American Community Survey (ACS)
  ...
  + Consists of samples every year or every five years
    + One year estimates are more current but have a larger margin of error, making five year estimates more valuable[@stanford]
...
Before using Tidycensus you need to get a Census Bureau API key and activate it. This can be done by completing these steps one time:[@census]
...
After completing these steps, you will always have access to the Census Bureau API. 

The first step is to select the variables you want using the `load_variables(year, dataset = )` function. The options for the `year` and `dataset` parameters are outlined below:
...
- Decennial Census
  + `dataset = `
    ...
    + `"sf3"` and `"sf4"`: long form census data on detailed demographic information such as income and occupation
      + available only for 1790 to 2000, every 10 years
    + note portions of the 2020 census data is available 
...
- PEP
  + the PEP estimates are no available through the `load_variables()` function
...
In R scraping can be done through the `rvest` package. This is covered extensively [here](https://dcl-wrangle.stanford.edu/rvest.html) in the Stanford Data Wrangling book.[@stanford] This is a very useful tool and can be considered an entire branch of data science. 

## Conclusion

This chapter provides a comprehensive overview of data. By this point, using the skills above, you should be able to read almost any form of data into a data frame within R, manipulate and clean data, and get data off of the internet from tidycensus. These skills will give you a concrete foundation for statistical analysis and allow you to put data into the proper format in data frames for visualization. This book is meant to give a functional overview of these topics. Do not hesitate to look into [R for Data Science](https://r4ds.had.co.nz/index.html), [Stanford's Data Wrangling](https://dcl-wrangle.stanford.edu), and [Analyzing US Census Data](https://walker-data.com/census-r/index.html). These resources provide more in-depth explanations into these topics. 
